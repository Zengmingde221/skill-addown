...
流程：
  ...
  4. 读取 赛狐原表数据 下最新一份 广告组合_*.csv，portfolioId=广告组合ID → name，
...
周维度表 (广告位聚合_周维度_*.csv) 补充说明:
  分组键: 广告活动ID + 广告位 + 定位类型 + 日期所在周（周一为一周开始，周一~周日）
...
def main():
    ...
    header_cols_w = ["广告组合ID", NAME_COL, KEY_CAMPAIGN_NAME, KEY_CAMPAIGN_ID,
                     KEY_PLACEMENT, KEY_TARGETING, DATE_COL, MONTH_COL, WEEK_COL] + \
                    [n for n, _, _ in METRIC_COLS]
    with open(out_file_w, "w", encoding="utf-8-sig", newline="") as f:
        w = csv.writer(f)
        w.writerow(header_cols_w)
        for cname, cid, plc, tgt, drange, metrics, _n, mlabel, wlabel in results_w:
            pid = pid_of(cid)
