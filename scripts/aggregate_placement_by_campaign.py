#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""广告位维度分组聚合脚本（独立实现，仅标准库；供大模型数据分析用）

流程：
  1. 读取 筛选结果文件夹 下最新一份 广告位报告_*.xlsx（流式解析，表头行自动定位）
  2. 读取 筛选结果文件夹 下最新一份 广告活动报告_*.xlsx，仅取 广告活动ID→广告组合ID
     映射（广告位报告本身无 广告组合ID 列，需经活动ID关联）
  3. 按 [广告活动ID + 广告位 + 定位类型] 分组，按指标列 SUM / AVERAGE 聚合
  4. 读取 赛狐原表数据 下 广告组合信息查询表.csv（目录内最新一份 广告组合*.csv，
     兼容历史命名），portfolioId=广告组合ID → name，
     作为新列 [广告组合名称] 插入在 [广告组合ID] 之后（聚合完成后新增）
  5. 输出 CSV（UTF-8 BOM）到 广告分析聚合表 文件夹

数据完整性约束（不删除任何数据）：
  - 所有输入行全部参与聚合（各组行数总和 = 输入行数），不按任何条件丢弃行或组
  - 空值照口径处理：空单元格跳过该格累加（该行仍参与其他列）；空活动ID/空定位归入
    专用组；组合ID关联不到、组合名称匹配不到均保留该组、对应列留空
...
用法：
  python aggregate_placement_by_campaign.py
  python aggregate_placement_by_campaign.py --input <广告位报告.xlsx> --campaign <广告活动报告.xlsx> --portfolio <广告组合.csv> --out <输出目录>
...
# 分组键列（广告位报告无 广告组合ID 列，经 广告活动ID 关联广告活动报告获取）
...
# 广告活动报告中的关联列
CAMPAIGN_KEY_ID = "广告活动ID"
CAMPAIGN_KEY_PORTFOLIO = "广告组合ID"
...
WEEK_NOTES = """
周维度表 (广告位聚合_周维度_*.csv) 补充说明:
  分组键: 广告活动ID + 广告位 + 定位类型 + 日期所在周（以数据最晚日期为终点，
    倒数连续7天为一周；开头剩余天数不足7天时仍计为一周，保证最近一周永远满7天）
...
def load_xlsx_rows(path, header_marker, need_cols):
    ...
    # 表头定位: 前5行中找包含 header_marker 列名的行
    header_idx = None
    header = {}
...
# ── 活动ID → 组合ID 映射（来自广告活动报告）────────────────────────
def build_campaign_portfolio_map(campaign_file):
    """读广告活动报告，返回 {广告活动ID: 广告组合ID}。

    同一活动ID对应多个组合ID时合并为 "A|B" 并打印提示（不删数据）。
    """
    need = [CAMPAIGN_KEY_ID, CAMPAIGN_KEY_PORTFOLIO]
    _, data = load_xlsx_rows(campaign_file, CAMPAIGN_KEY_PORTFOLIO, need)
    mapping = {}
    multi = {}
    for row in data:
        cid = (row.get(CAMPAIGN_KEY_ID) or "").strip()
        pid = (row.get(CAMPAIGN_KEY_PORTFOLIO) or "").strip()
        if not cid:
            continue
        ...
        if prev is None:
            ...
            merged = {prev, pid} - {""}
            mapping[cid] = "|".join(sorted(merged))
            multi[cid] = mapping[cid]
    if multi:
        print(f"提示: {len(multi)} 个活动ID在活动报告中对应多个组合ID，已合并（如 "
              f"{list(multi.items())[0]}）")
    print(f"✓ 活动ID→组合ID 映射: {len(mapping)} 个活动")
    return mapping
...
def aggregate(data, weekly=False):
    """按 (广告活动ID, 广告位, 定位类型) 分组聚合（weekly=True 时追加
    "日期所在周"为附加分组键，以数据最晚日期为终点倒数连续7天为一周）。

    返回结果列表[(campaign_name, campaign_id, placement, targeting, 日期范围,
    ...
    for r_idx, row in enumerate(data, start=1):
        ...
        plc = (row.get(KEY_PLACEMENT) or "").strip()
        ...
        key = (cid, plc, tgt, wk) if weekly else (cid, plc, tgt)
        g = groups.setdefault(
            key, {"names": set(), "sum": {}, "avg_sum": {}, "avg_n": {},
                  "dmin": None, "dmax": None, "dates": set(), "n": 0})
        if d is not None:
    ...
    if empty_cid_rows:
        print(f"提示: {empty_cid_rows} 行广告活动ID为空，已归入空ID组（不删行）")
    ...
    for gkey, g in groups.items():
        ...
        if weekly:
            ...
            results.append((campaign_name, cid, plc, tgt, drange, out, g["n"],
                            mlabel, ndays))
        else:
            cid, plc, tgt = gkey
            results.append((campaign_name, cid, plc, tgt, "", out, g["n"]))

    # 排序: 按活动名称、广告位、定位类型（稳定可复现）；周维度再按日期范围
    if weekly:
        results.sort(key=lambda t: (t[0], t[2], t[3], t[4]))
    else:
        results.sort(key=lambda t: (t[0], t[2], t[3]))
    return results
...
def parse_args():
    ap = argparse.ArgumentParser(
        description="广告位报告按 [广告活动ID+广告位+定位类型] 分组聚合 → CSV")
    ap.add_argument("--input", default=None,
                    help=f"广告位报告xlsx（默认取 {DEFAULT_INPUT_DIR} 下最新一份）")
    ap.add_argument("--campaign", default=None,
                    help=f"广告活动报告xlsx，用于活动ID→组合ID关联（默认取最新一份）")
...
def main():
    ...
    in_file = Path(args.input) if args.input else pick_input(DEFAULT_INPUT_DIR, "广告位报告", ".xlsx")
    cp_file = Path(args.campaign) if args.campaign else pick_input(DEFAULT_INPUT_DIR, "广告活动报告", ".xlsx")
    pf_file = Path(args.portfolio) if args.portfolio else latest_file(DEFAULT_PORTFOLIO_DIR, "广告组合", ".csv")
    if in_file is None:
        fail(f"{DEFAULT_INPUT_DIR} 下未找到 广告位报告.xlsx（或历史 广告位报告_*.xlsx）")
    if cp_file is None:
        fail(f"{DEFAULT_INPUT_DIR} 下未找到 广告活动报告.xlsx（或历史 广告活动报告_*.xlsx，组合ID关联需要）")
    if pf_file is None:
    ...
    need = [KEY_CAMPAIGN_ID, KEY_PLACEMENT, KEY_TARGETING, KEY_CAMPAIGN_NAME, DATE_COL] + \
           [n for n, _, _ in METRIC_COLS if n not in ("ACoS", "ROAS")]
    header, data = load_xlsx_rows(in_file, KEY_PLACEMENT, need)
    print(f"✓ 读取完成: {len(data)} 行数据, {len(header)} 列")

    cp_map = build_campaign_portfolio_map(cp_file)
    ...
    # 组合ID关联 + 组合名称匹配
    unmatched_cp = [t for t in results if t[1] not in cp_map]
    if unmatched_cp:
        print(f"提示: {len(unmatched_cp)} 组的活动ID在活动报告中未找到，组合ID/名称留空（不删组）")
    names = load_portfolio_names(pf_file)

    def pid_of(cid):
        return cp_map.get(cid, "")

    matched = sum(1 for t in results if pid_of(t[1]) in names and pid_of(t[1]) != "")
    print(f"✓ 组合ID关联: {len(results) - len(unmatched_cp)}/{len(results)}；"
          f"名称匹配: {matched}/{len(results)}")

    out_dir = Path(args.out)
    out_dir.mkdir(parents=True, exist_ok=True)
    # 输出命名 = 固定名（同类型仅一个文件；同名存在时直接覆盖，聚合表目录不累积旧文件）
    out_file = out_dir / "广告位聚合.csv"
    if out_file.exists():
        print(f"提示: 输出文件已存在，覆盖写入 → {out_file.name}")

    header_cols = ["广告组合ID", NAME_COL, KEY_CAMPAIGN_NAME, KEY_CAMPAIGN_ID,
                   KEY_PLACEMENT, KEY_TARGETING] + [n for n, _, _ in METRIC_COLS]
    with open(out_file, "w", encoding="utf-8-sig", newline="") as f:
        w = csv.writer(f)
        w.writerow(header_cols)
        for cname, cid, plc, tgt, _dr, metrics, _n in results:
            pid = pid_of(cid)
            w.writerow([pid, names.get(pid, ""), cname, cid, plc, tgt] +
                       [fmt_num(metrics.get(m), fmt) for m, _, fmt in METRIC_COLS])

    out_file_w = out_dir / "广告位聚合_周维度.csv"
    header_cols_w = ["广告组合ID", NAME_COL, KEY_CAMPAIGN_NAME, KEY_CAMPAIGN_ID,
                     KEY_PLACEMENT, KEY_TARGETING, DATE_COL, MONTH_COL,
                     DAYS_COL] + \
                    [n for n, _, _ in METRIC_COLS]
    with open(out_file_w, "w", encoding="utf-8-sig", newline="") as f:
        w = csv.writer(f)
        w.writerow(header_cols_w)
        for cname, cid, plc, tgt, drange, metrics, _n, mlabel, ndays in results_w:
            pid = pid_of(cid)
            w.writerow([pid, names.get(pid, ""), cname, cid, plc, tgt, drange,
                        mlabel, ndays] +
                       [fmt_num(metrics.get(m), fmt) for m, _, fmt in METRIC_COLS])
