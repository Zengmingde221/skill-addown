#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""搜索词维度分组聚合脚本（独立实现，仅标准库；供大模型数据分析用）

流程：
  1. 读取 赛狐原表筛选结果文件夹 下 搜索词报告.xlsx（固定名优先，缺失回退最新一份；下载阶段已按活动ID关联筛选，口径与其他维度一致）
  2. 读取 筛选结果文件夹 下最新一份 广告活动报告_*.xlsx，仅取 广告活动ID→广告组合ID
     映射（搜索词报告本身无 广告组合ID 列，需经活动ID关联）
  3. 按 [用户搜索词 + 匹配类型 + 投放 + 广告活动ID + 定位类型] 分组，
     按指标列 SUM / AVERAGE 聚合
  4. 广告组合名称 = 搜索词报告原生列（组内保留唯一值，多值合并 "A|B"）；
     广告组合ID = 关联新增列；广告组合表CSV仅用于关联结果校验
  5. 输出 CSV（UTF-8 BOM）到 广告分析聚合表 文件夹

数据完整性约束（不删除任何数据）：
  - 所有输入行全部参与聚合（各组行数总和 = 输入行数），不按任何条件丢弃行或组
  - 空值照口径处理：空单元格跳过该格累加（该行仍参与其他列）；空键（如自动行的
    空匹配类型、空投放）归入专用组；组合ID关联不到则该组保留、对应列留空
...
用法：
  python aggregate_searchterm_by_campaign.py
  python aggregate_searchterm_by_campaign.py --input <搜索词报告.xlsx> --campaign <广告活动报告.xlsx> --portfolio <广告组合.csv> --out <输出目录>
...
DEFAULT_INPUT_DIR = BASE_DIR / "赛狐原表筛选结果文件夹"   # 搜索词报告筛选版（活动ID关联）
DEFAULT_RAW_DIR = BASE_DIR / "赛狐原表数据"               # 旧版无筛选原表兜底
DEFAULT_CAMPAIGN_DIR = BASE_DIR / "赛狐原表筛选结果文件夹"
DEFAULT_PORTFOLIO_DIR = BASE_DIR / "赛狐原表数据"
DEFAULT_OUT_DIR = BASE_DIR / "广告分析聚合表"

# 分组键列（搜索词报告无 广告组合ID 列，经 广告活动ID 关联广告活动报告获取）
KEY_TERM = "用户搜索词"
KEY_MATCH = "匹配类型"
KEY_TARGET = "投放"
...
# 输出类型: num=纯数字2位, pct=百分比(×100带%), acos/roas=推导列
METRIC_COLS = [
    ("广告花费", "sum", "num"),
    ("广告曝光量", "sum", "num"),
    ("广告点击量", "sum", "num"),
...
def aggregate(data, weekly=False):
    ...
    for r_idx, row in enumerate(data, start=1):
        ...
        term = (row.get(KEY_TERM) or "").strip()
        match = (row.get(KEY_MATCH) or "").strip()
        target = (row.get(KEY_TARGET) or "").strip()
        tgt = (row.get(KEY_TARGETING) or "").strip()
        cname = (row.get(KEY_CAMPAIGN_NAME) or "").strip()
        pname = (row.get(NAME_COL) or "").strip()
        for k, v in ((KEY_TERM, term), (KEY_MATCH, match), (KEY_TARGET, target),
    ...
    for gkey, g in groups.items():
        ...
        if len(g["pnames"]) > 1:
            multi_pname_groups += 1
        pname = " | ".join(sorted(g["pnames"])) if g["pnames"] else ""
        if weekly:
            ...
            results.append((term, match, target, cid, tgt,
                            campaign_name, pname, "", out, g["n"]))
    if multi_pname_groups:
        print(f"提示: {multi_pname_groups} 组内原生「广告组合名称」存在多个值，已按 | 合并")

    # 排序: 按活动名称、用户搜索词、匹配类型、投放、定位类型（稳定可复现）；
    # 周维度再按日期范围
    if weekly:
        results.sort(key=lambda t: (t[5], t[0], t[1], t[2], t[4], t[7]))
    else:
        results.sort(key=lambda t: (t[5], t[0], t[1], t[2], t[4]))
    return results


def load_portfolio_names(path):
    """读广告组合CSV → {portfolioId: name}（本脚本仅用于关联结果校验）。"""
    names = {}
...
def parse_args():
    ap = argparse.ArgumentParser(
        description="搜索词报告按 [用户搜索词+匹配类型+投放+广告活动ID+定位类型] 分组聚合 → CSV")
    ap.add_argument("--input", default=None,
                    help=f"搜索词报告xlsx（默认取 {DEFAULT_INPUT_DIR} 下最新一份）")
    ap.add_argument("--campaign", default=None,
                    help=f"广告活动报告xlsx，用于活动ID→组合ID关联（默认取最新一份）")
    ap.add_argument("--portfolio", default=None,
                    help=f"广告组合CSV，用于关联结果校验（默认取最新一份）")
...
def main():
    ...
    print(f"  输入表   : {in_file}")
    print(f"  活动表   : {cp_file}")
    print(f"  组合表   : {pf_file}（仅校验用）")
    print(f"  输出目录 : {args.out}")
    print("=" * 60)

    need = [KEY_TERM, KEY_MATCH, KEY_TARGET, KEY_CAMPAIGN_ID, KEY_TARGETING,
            KEY_CAMPAIGN_NAME, NAME_COL, DATE_COL] + \
           [n for n, _, _ in METRIC_COLS if n not in ("ACoS", "ROAS")]
    header, data = load_xlsx_rows(in_file, KEY_TERM, need)
    print(f"✓ 读取完成: {len(data)} 行数据, {len(header)} 列")

    cp_map = build_campaign_portfolio_map(cp_file)

    results = aggregate(data)
    total_group_rows = sum(t[9] for t in results)
    ...
    # 组合ID关联 + 组合表校验
    unmatched_cp = [t for t in results if t[3] not in cp_map]
    if unmatched_cp:
        print(f"提示: {len(unmatched_cp)} 组的活动ID在活动报告中未找到，组合ID留空（不删组）")
    ...
    # 校验: 关联出的组合ID应在组合表中存在
    pid_in_pf = sum(1 for t in results if pid_of(t[3]) and pid_of(t[3]) in names)
    pid_out_pf = [t for t in results if pid_of(t[3]) and pid_of(t[3]) not in names]
    if pid_out_pf:
        print(f"提示: {len(pid_out_pf)} 组的组合ID在组合表中无对应记录（仍保留ID值）")
    print(f"✓ 组合ID关联: {len(results) - len(unmatched_cp)}/{len(results)}；"
          f"组合表校验通过: {pid_in_pf}/{len(results)}")

    out_dir = Path(args.out)
    out_dir.mkdir(parents=True, exist_ok=True)
    # 输出命名 = 固定名（同类型仅一个文件；同名存在时直接覆盖，聚合表目录不累积旧文件）
    out_file = out_dir / "搜索词聚合.csv"
    if out_file.exists():
        print(f"提示: 输出文件已存在，覆盖写入 → {out_file.name}")

    header_cols = [PID_COL, NAME_COL, KEY_CAMPAIGN_NAME, KEY_CAMPAIGN_ID,
                   KEY_TARGETING, KEY_MATCH, KEY_TARGET, KEY_TERM] + \
                  [n for n, _, _ in METRIC_COLS]
    with open(out_file, "w", encoding="utf-8-sig", newline="") as f:
        w = csv.writer(f)
        w.writerow(header_cols)
        for term, match, target, cid, tgt, cname, pname, _dr, metrics, _n in results:
