...
周维度表 (广告组合聚合_周维度_*.csv) 补充说明:
  分组键: 广告组合ID + 日期所在周（周一定义为一周开始，周一~周日）
...
def aggregate(data, weekly=False):
    ...
    周一为一周开始）。返回 [(组键, 日期范围, 指标dict, 行数)]。
    日期范围仅周维度有意义（全量模式恒为空串）。fail-fast 非数值/非日期。"""
    groups = {}   # key -> {"sum": {...}, "avg_sum": {...}, "avg_n": {...}, "dmin", "dmax", "n"}
    empty_id_rows = 0
    empty_date_rows = 0
    bad_cells = []
    ...
    for gkey in groups:
        ...
        if weekly:
            ...
            results.append((pid, drange, out, g["n"], mlabel, wlabel))
        else:
...
def main():
    ...
    header_cols_w = [GROUP_COL, NAME_COL, DATE_COL, MONTH_COL, WEEK_COL] + \
                    [n for n, _, _ in METRIC_COLS]
    with open(out_file_w, "w", encoding="utf-8-sig", newline="") as f:
        w = csv.writer(f)
        w.writerow(header_cols_w)
        for key, drange, metrics, _n, mlabel, wlabel in results_w:
            w.writerow([key, names.get(key, ""), drange, mlabel, wlabel] +
                       [fmt_num(metrics.get(n), fmt) for n, _, fmt in METRIC_COLS])
