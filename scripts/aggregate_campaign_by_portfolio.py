#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""广告组合维度分组聚合脚本（独立实现，仅标准库；供大模型数据分析用）

流程：
  1. 读取 筛选结果文件夹 下最新一份 广告活动报告_*.xlsx（流式解析，表头行自动定位）
  2. 按 [广告组合ID] 列分组，按指标列 SUM / AVERAGE 聚合
  3. 读取 赛狐原表数据 下 广告组合信息查询表.csv（目录内最新一份 广告组合*.csv，
     兼容历史命名），portfolioId=广告组合ID → name，
     作为新列 [广告组合名称] 插入在 [广告组合ID] 之后
...
用法：
  python aggregate_campaign_by_portfolio.py
  python aggregate_campaign_by_portfolio.py --input <广告活动报告.xlsx> --portfolio <广告组合.csv> --out <输出目录>

退出码 0 = 成功；1 = 文件缺失/表头缺失/聚合列缺失等（fail-fast）
"""
...
WEEK_NOTES = """
周维度表 (广告组合聚合_周维度_*.csv) 补充说明:
  分组键: 广告组合ID + 日期所在周（以数据最晚日期为终点，倒数连续7天为一周；
    开头剩余天数不足7天时仍计为一周，保证最近一周永远满7天）
...
def load_xlsx_rows(path):
    ...
    表头行不固定在第1行：扫描前5行，取包含 GROUP_COL 的第一行作为表头。
    ...
    if header_idx is None:
        fail(f"前5行未找到含「{GROUP_COL}」的表头行: {path}")

    # 校验聚合所需列全部存在（ACoS/ROAS 由花费/销售额推导，不要求列本身存在）
    need = [GROUP_COL, DATE_COL] + [n for n, _, _ in METRIC_COLS if n not in ("ACoS", "ROAS")]
...
def aggregate(data, weekly=False):
    """按 广告组合ID 分组聚合（weekly=True 时追加"日期所在周"为附加分组键，
    以数据最晚日期为终点倒数连续7天为一周）。返回 [(组键, 日期范围, 指标dict, 行数)]。
    日期范围仅周维度有意义（全量模式恒为空串）。fail-fast 非数值/非日期。"""
    ...
    groups = {}   # key -> {"sum": {...}, "avg_sum": {...}, "avg_n": {...}, "dmin", "dmax", "dates", "n"}
    empty_id_rows = 0
    empty_date_rows = 0
    bad_cells = []
    for r_idx, row in enumerate(data, start=1):
        ...
        gkey = (key, wk) if weekly else key
        g = groups.setdefault(gkey, {"sum": {}, "avg_sum": {}, "avg_n": {},
                                     "dmin": None, "dmax": None,
                                     "dates": set(), "n": 0})
        if d is not None:
    ...
    if empty_id_rows:
        print(f"提示: {empty_id_rows} 行广告组合ID为空，已归入空ID组")
    ...
    for gkey in groups:
        ...
        if weekly:
            ...
            results.append((pid, drange, out, g["n"], mlabel, ndays))
        else:
            results.append((gkey, "", out, g["n"]))
    # 空ID组排最后，其余按ID排序（周维度再按日期范围）
    if weekly:
        results.sort(key=lambda t: (t[0] == "", t[0], t[1]))
    else:
        results.sort(key=lambda t: (t[0] == "", t[0]))
    return results
...
# ── 入口 ───────────────────────────────────────────────────────────
def parse_args():
    ap = argparse.ArgumentParser(description="广告活动报告按广告组合ID分组聚合 → CSV")
...
def main():
    ...
    out_file = out_dir / "广告组合聚合.csv"
    if out_file.exists():
        print(f"提示: 输出文件已存在，覆盖写入 → {out_file.name}")

    header_cols = [GROUP_COL, NAME_COL] + [n for n, _, _ in METRIC_COLS]
    with open(out_file, "w", encoding="utf-8-sig", newline="") as f:
        w = csv.writer(f)
        w.writerow(header_cols)
        for key, _dr, metrics, _n in results:
            w.writerow([key, names.get(key, "")] +
                       [fmt_num(metrics.get(n), fmt) for n, _, fmt in METRIC_COLS])

    out_file_w = out_dir / "广告组合聚合_周维度.csv"
    header_cols_w = [GROUP_COL, NAME_COL, DATE_COL, MONTH_COL, DAYS_COL] + \
                    [n for n, _, _ in METRIC_COLS]
    with open(out_file_w, "w", encoding="utf-8-sig", newline="") as f:
        w = csv.writer(f)
        w.writerow(header_cols_w)
        for key, drange, metrics, _n, mlabel, ndays in results_w:
            w.writerow([key, names.get(key, ""), drange, mlabel, ndays] +
                       [fmt_num(metrics.get(n), fmt) for n, _, fmt in METRIC_COLS])
