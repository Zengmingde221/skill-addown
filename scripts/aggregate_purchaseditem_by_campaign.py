...
流程：
  ...
  3. 按 [广告活动ID + ASIN + SKU + 投放 + 定位类型] 分组聚合：
       - 广告活动      : 组内保留唯一值（多值按 | 合并，不删行）
       - 其他ASIN      : 组内去重，按首次出现顺序以 ; 拼接
       - 其他SKU销售额 : SUM，保留 2 位小数
  4. 广告组合ID / 广告组合名称 = 关联新增列（活动ID未匹配 → 留空，不删组）
...
用法：
  ...
  python aggregate_purchaseditem_by_campaign.py --input <购买商品报告.xlsx> --campaign-agg <广告活动聚合.csv> --out <输出目录>
...
def aggregate(data):
    ...
    返回 (results, raw_sales_total)：
      results = [(campaign_id, asin, sku, target, targeting, campaign_name,
                  other_asin_joined, sales_sum, n_rows)]
    所有行全部计入（空键归入空字符串键组），零删行；非数值 fail-fast。
    """
    groups = {}   # key -> {"names": set, "asins": list(有序去重), "sales": float, "n": int}
    ...
    for r_idx, row in enumerate(data, start=1):
        ...
        v = to_float(row.get(COL_OTHER_SALES, ""))
        if v is None:
            s = (row.get(COL_OTHER_SALES) or "").strip()
            if s and s != "-":
                bad_cells.append((r_idx, s))
        else:
            g["sales"] += v
            raw_sales_total += v
        g["n"] += 1
    if bad_cells:
        r, s = bad_cells[0]
        fail(f"第{r}行列「{COL_OTHER_SALES}」存在非数值文本 {s!r}"
    ...
    return results, raw_sales_total
...
def main():
    ...
    need = [KEY_CAMPAIGN_ID, KEY_ASIN, KEY_SKU, KEY_TARGET, KEY_TARGETING,
            KEY_CAMPAIGN_NAME, COL_OTHER_ASIN, COL_OTHER_SALES]
    ...
    results, raw_sales_total = aggregate(data)
    ...
    header_cols = [NAME_COL, PID_COL, KEY_CAMPAIGN_NAME, KEY_CAMPAIGN_ID,
                   ...
                   COL_OTHER_ASIN, COL_OTHER_SALES]
    with open(out_file, "w", encoding="utf-8-sig", newline="") as f:
        w = csv.writer(f)
        w.writerow(header_cols)
        for cid, asin, sku, target, tgt, cname, oasins, sales, _n in results:
            pid, pname = cp_map.get(cid, ("", ""))
            w.writerow([pname, pid, cname, cid, tgt, asin, sku, target,
                        oasins, f"{round(sales, 2):.2f}"])
